...
"""
Standalone Direct Synchronizer & Database Test Harness (Non-ROS 2 Version)

Purpose in Pipeline:
    Serves as the standalone, non-ROS 2 ("no_noeud") execution harness designed 
    to validate database schema constraints, client authentication workflows, and 
    frame-level multi-rate telemetry ingestion directly through psycopg2 before 
    deploying nodes into the ROS 2 executor environment.

Key Functionality:
    - Direct Non-Middleware Execution ("no_noeud"): Executes pure Python database 
      ingestion without ROS 2 topic overhead to establish baseline performance metrics.
    - Relational Client & Mission Ingestion: Handles lookup/creation in `public.clients` 
      and creates corresponding dive entries in `public.missions`.
    - Multi-Rate Telemetry Simulation: Emulates multi-rate sensors (10 Hz depth/distance, 
      1 Hz USBL positioning) synchronized to a 30 FPS video stream using modulo gating.
    - Spatial-Temporal Database Writing: Encapsulates WGS 84 coordinates into PostGIS 
      `ST_SetSRID(ST_MakePoint(lon, lat), 4326)` points and writes frame-indexed records 
